...
const DietPlanViewer = ({ dietPlan }) => {
  ...
  const createMealPages = () => {
    const pages = [];
    const pageHeight = 356 * 3.5; // Legal size height in pixels (approximate)
    ...
    mealSections.forEach((mealSection) => {
      ...
      mealOptions.forEach((option, optionIndex) => {
        ...
        if (
          currentPageHeight + optionHeight > availableHeight &&
          currentPage.length > 0
        ) {
          pages.push({
            ...mealSection,
            pageTitle:
              totalOptions > currentPage.length
                ? `${mealSection.mealName} (${pageIndex + 1}/${Math.ceil(
                    totalOptions / currentPage.length
                  )})`
